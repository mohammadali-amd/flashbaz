...
const OrderPage = () => {
   ...
   return (
      <div className='mx-6 lg:mx-20 my-5'>
         <h2 className='text-4xl my-8 font-medium truncate'>
            شماره سفارش {order._id}
         </h2>

         <div className='lg:flex lg:justify-between lg:gap-14'>
            <div className='lg:w-3/4 space-y-8 border border-stone-200 shadow-lg  shadow-gray-300 rounded-xl py-8 px-10'>
               {/* User Info */}
               <div>
                  <h2>مشخصات کاربر</h2>
                  ...
                  {order.isDelivered ? (
                     <p className='flex justify-between items-center lg:w-fit text-lg bg-green-300 text-green-700 border border-green-700 rounded-md p-4 my-4'>
                        در تاریخ {order.deliveredAt} ارسال شده
                     </p>
                  ) : (
                     <div className='lg:flex gap-3'>
                        <p className='flex justify-between items-center text-lg bg-red-300 text-red-800 border border-red-800 rounded-md p-4 my-4'>
                           ارسال نشده
                        </p>
                        {userInfo && userInfo.isAdmin && order.isPaid && !order.isDelivered && (
                           <button onClick={deliverOrderHandler} className='flex justify-between items-center w-full lg:w-fit text-lg bg-cyan-300 text-cyan-800 border border-cyan-800 rounded-md p-4 my-4'>
                              ارسال شد ✔
                           </button>
                        )}
                     </div>
               ...
               <div>
                  ...
                  {order.isPaid ? (
                     ...
                     <p className='flex justify-between items-center lg:w-fit text-lg bg-red-300 text-red-800 border border-red-800 rounded-md p-4 my-4'>
                        پرداخت نشده
                     </p>
                  )}
               </div>
